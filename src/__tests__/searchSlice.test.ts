...
describe('fetchResults async thunk', () => {
  ...
  describe('fulfilled — first page (append: false)', () => {
    it('replaces results and sets status to succeeded', () => {
      const payload = fulfilledPayload({ results: [makeTrack(), makeAlbum()], totalResults: 50 });
      const action = fetchResults.fulfilled(payload, '', { term: 'test', offset: 0 });
      const state = searchReducer(initialState, action);
    ...
    it('sets hasMore true when results.length < totalResults', () => {
      const payload = fulfilledPayload({ results: [makeTrack()], totalResults: 30 });
      const action = fetchResults.fulfilled(payload, '', { term: 'test', offset: 0 });
      const state = searchReducer(initialState, action);
    ...
    it('sets hasMore false when all results are loaded', () => {
      const payload = fulfilledPayload({ results: [makeTrack()], totalResults: 1 });
      const action = fetchResults.fulfilled(payload, '', { term: 'test', offset: 0 });
      const state = searchReducer(initialState, action);
      expect(state.hasMore).toBe(false);
    });
    ...
    it('clears error on success', () => {
      const stateWithError = { ...initialState, error: 'previous error', status: 'failed' as const };
  ...
  describe('fulfilled — pagination (append: true)', () => {
    ...
    it('appends new unique results', () => {
      ...
      const payload = fulfilledPayload({ results: [newTrack], totalResults: 40, append: true });
      const action = fetchResults.fulfilled(payload, '', { term: 'test', offset: 10, append: true });
    ...
    it('deduplicates tracks that are already in results', () => {
      ...
      const payload = fulfilledPayload({ results: [duplicate], totalResults: 40, append: true });
      const action = fetchResults.fulfilled(payload, '', { term: 'test', offset: 10, append: true });
      const state = searchReducer(stateWithResults, action);
    ...
    it('increments the page counter', () => {
      const payload = fulfilledPayload({ results: [makeTrack({ trackId: 99 })], totalResults: 40, append: true });
      const action = fetchResults.fulfilled(payload, '', { term: 'test', offset: 10, append: true });
      const state = searchReducer(stateWithResults, action);
    ...
    it('sets hasMore false when accumulated results reach total', () => {
      ...
      const payload = fulfilledPayload({ results: newTracks, totalResults: 3, append: true });
      const action = fetchResults.fulfilled(payload, '', { term: 'test', offset: 10, append: true });
      const state = searchReducer(stateWithResults, action);
      expect(state.hasMore).toBe(false);
    });
